...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentCountry = '<?php echo e($country); ?>';
    ...
    function filterProducts(country, search = '') {
        ...
        // Make AJAX request
        fetch('<?php echo e(route("shop.filter")); ?>', {
            method: 'POST',
            headers: {
    ...
    function renderProducts(products) {
        ...
        products.forEach((product, index) => {
            ...
            const imageSrc = product.image_path ? `<?php echo e(asset('')); ?>${product.image_path}` : `https://via.placeholder.com/320x180?text=${encodeURIComponent(product.name || 'Product')}`;
...
<?php $__env->startSection('title', 'Cửa hàng - Sky Music Store'); ?>
...
<?php $__env->startSection('content'); ?>
 <div id="shop" class="page-content" x-data="{ showDetail: false, product: {} }">
                ...
                <div class="max-w-md mx-auto mb-12">
                    <div class="relative">
                        ...
                        <button id="clear-search" 
                                class="absolute right-12 top-1/2 transform -translate-y-1/2 text-gray-300 hover:text-white transition-colors <?php echo e(empty($search) ? 'hidden' : ''); ?>"
                                title="Xóa tìm kiếm">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div id="search-info" class="mt-2 text-center text-blue-200 text-sm <?php echo e(empty($search) ? 'hidden' : ''); ?>">
                        <?php if(!empty($search)): ?>
                            Tìm kiếm cho: "<?php echo e($search); ?>" - <?php echo e($products->count()); ?> kết quả
                        <?php endif; ?>
                    </div>
                </div>
                        ...
                        <div class="game-card rounded-xl p-4">
                            <div class="bg-gradient-to-br from-blue-400 to-purple-500 rounded-lg mb-4 flex items-center justify-center" style="aspect-ratio: 16/9; width: 100%;">
                                <?php if(!empty($product['image_path'])): ?>
                                    <img src="<?php echo e(asset($product['image_path'])); ?>" alt="<?php echo e($product['name'] ?? 'Sản phẩm'); ?>" class="w-full h-full object-cover rounded-lg">
                                <?php else: ?>
                                    <span class="text-3xl">
                                        <?php if(str_contains($product['country_region'] ?? '', 'Việt Nam')): ?>
                                            🇻🇳
                                        <?php elseif(str_contains($product['country_region'] ?? '', 'Hàn Quốc')): ?>
                                            🇰🇷
                                        <?php elseif(str_contains($product['country_region'] ?? '', 'Nhật Bản')): ?>
                                            🇯🇵
                                        <?php elseif(str_contains($product['country_region'] ?? '', 'Trung Quốc')): ?>
                                            🇨🇳
                                        <?php else: ?>
                                            🎵
                                        <?php endif; ?>
                            ...
                            <h4 class="orbitron font-bold text-white mb-2"><?php echo e($product['name'] ?? 'Chưa có tên'); ?></h4>
                            <p class="inter text-blue-200 text-sm mb-1">Tác giả: <?php echo e($product['author'] ?? 'Chưa xác định'); ?></p>
                            <p class="inter text-blue-200 text-sm mb-1">Người soạn: <?php echo e($product['transcribed_by'] ?? 'Admin'); ?></p>
                            <div class="flex justify-between items-center mt-2">
                                <span class="orbitron text-yellow-300 font-bold"><?php echo e(number_format($product['price'] ?? 0)); ?>đ</span>
                                <button class="bg-blue-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-600 transition-colors"
                                    @click="product = { 
                                        name: '<?php echo e(addslashes($product['name'] ?? 'Chưa có tên')); ?>', 
                                        author: '<?php echo e(addslashes($product['author'] ?? 'Chưa xác định')); ?>', 
                                        composer: '<?php echo e(addslashes($product['transcribed_by'] ?? 'Admin')); ?>', 
                                        price: '<?php echo e(number_format($product['price'] ?? 0)); ?>đ', 
                                        img: '<?php echo e(!empty($product['image_path']) ? asset($product['image_path']) : 'https://via.placeholder.com/320x180?text=' . urlencode($product['name'] ?? 'Product')); ?>', 
                                        video: '<?php echo e($product['youtube_demo_url'] ?? 'https://www.youtube.com/embed/dQw4w9WgXcQ'); ?>',
                                        seller_id: '<?php echo e($product['seller_id'] ?? ''); ?>',
                                        product_id: '<?php echo e(isset($product['id']) ? $product['id'] : ''); ?>'
                                    }; showDetail = true;">
                                    Xem
                                </button>
                            </div>
                    ...
                    <div class="flex justify-center mt-8">
                        <div class="flex gap-2">
                            <?php for($i = 1; $i <= ceil($totalProducts / $perPage); $i++): ?>
                                <button class="px-4 py-2 rounded-lg <?php echo e($currentPage == $i ? 'bg-blue-500 text-white' : 'bg-white bg-opacity-20 text-white hover:bg-opacity-30'); ?> transition-colors"
                                        onclick="window.location.href='<?php echo e(request()->fullUrlWithQuery(['page' => $i])); ?>'">
                                    <?php echo e($i); ?>
    ...
    function addToCart() {
        ...
        var currentUserId = '<?php echo e($user_id); ?>';
        if (shopDiv && shopDiv._x_dataStack) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\app\laragon\www\cayvlon\PTTKPM25-26_ClassN05_Nhom6\SRC\SM_Store\resources\views/page/shop/index.blade.php ENDPATH**/ ?>