...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('account.update') }}" method="POST" enctype="multipart/form-data" class="bg-white/30 backdrop-blur-lg rounded-2xl shadow-xl p-8 space-y-6 border border-white/20">
        @csrf
        @method('PUT')
        
        <!-- Current Avatar Display -->
        <div class="text-center mb-6">
            <img src="{{ isset($currentUser) ? $currentUser['avatar'] : (isset($userData) ? $userData['avatar'] : session('avatar', '/img/default-avatar.png')) }}" 
                 alt="Avatar" 
                 id="current-avatar"
        ...
        <div>
            ...
            <input type="text" id="name" name="name" 
                   value="{{ old('name', isset($currentUser) ? $currentUser['name'] : (isset($userData) ? $userData['name'] : session('name', ''))) }}" 
        ...
        <div>
            ...
            <input type="email" id="email" 
                   value="{{ isset($currentUser) ? $currentUser['email'] : (isset($userData) ? $userData['email'] : session('email', '')) }}" 
