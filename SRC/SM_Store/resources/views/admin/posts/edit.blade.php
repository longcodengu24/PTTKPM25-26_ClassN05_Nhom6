...
<div class="admin-content active px-6 pb-6 flex justify-center">
    <div class="admin-card rounded-xl p-8 w-full max-w-2xl">
        <h2 class="orbitron text-2xl font-bold text-white mb-6">Sửa Bài Viết</h2>
        <form enctype="multipart/form-data" method="POST" action="">
            @csrf
            @method('PUT')
            <div class="mb-5">
                <label class="block text-gray-300 font-semibold mb-2">Tiêu đề</label>
                <input type="text" name="title" class="bg-white bg-opacity-20 text-white px-4 py-2 rounded-lg border border-white border-opacity-30 w-full placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Nhập tiêu đề bài viết" value="{{ old('title', $post->title ?? '') }}">
            </div>
            <div class="mb-5">
                <label class="block text-gray-300 font-semibold mb-2">Tóm tắt</label>
                <textarea name="summary" rows="3" class="bg-white bg-opacity-20 text-white px-4 py-2 rounded-lg border border-white border-opacity-30 w-full placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="Nhập tóm tắt ngắn gọn về bài viết...">{{ old('summary', $post->summary ?? '') }}</textarea>
            </div>
            <div class="mb-5">
                <label class="block text-gray-300 font-semibold mb-2">Trạng thái bài viết</label>
                <div class="relative" x-data="{ open: false, selected: '{{ old('status', ($post->status ?? 'visible') === 'visible' ? 'Hiển thị' : 'Ẩn') }}', options: ['Hiển thị', 'Ẩn'] }">
            ...
            <div class="mb-5">
                ...
                <textarea id="tinymce-content" name="content" rows="12" class="bg-white bg-opacity-20 text-white px-4 py-2 rounded-lg border border-white border-opacity-30 w-full placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300 indent-8" placeholder="Nhập nội dung bài viết...">{{ old('content', $post->content ?? '') }}</textarea>
            </div>
