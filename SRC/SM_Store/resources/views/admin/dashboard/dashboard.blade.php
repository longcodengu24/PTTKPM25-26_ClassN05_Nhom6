...
<div class="p-6">
                    ...
                    <div>
                        <div class="flex items-center space-x-2 mb-2">
                            ...
                            <div class="bg-green-500 bg-opacity-20 px-2 py-1 rounded">
                                <span class="text-green-400 text-xs font-bold">{{ $stats['commission_rate'] ?? 30 }}%</span>
                            </div>
                        </div>
                        ...
                        <p class="text-gray-400 text-xs mt-2">
                            <span class="text-green-400">✅</span> Từ {{ number_format($stats['total_sales'] ?? 0) }} VND tổng doanh số bán hàng
                        </p>
                    </div>
                </div>
                <div class="text-right">
                    <div class="bg-gray-800 bg-opacity-50 rounded-lg p-4">
                        <p class="text-gray-400 text-xs mb-2">Tỷ lệ hoa hồng</p>
                        <p class="text-2xl font-bold text-green-400">{{ $stats['commission_rate'] ?? 30 }}%</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="admin-card rounded-xl p-6 bg-gradient-to-br from-blue-500 bg-opacity-10 border-blue-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-300 text-sm">Doanh thu hôm nay</p>
                    <p class="text-2xl font-bold text-white orbitron">{{ number_format($stats['today_revenue'] ?? 0) }}</p>
        ...
        <div class="admin-card rounded-xl p-6 bg-gradient-to-br from-purple-500 bg-opacity-10 border-purple-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-300 text-sm">Doanh thu tuần này</p>
                    <p class="text-2xl font-bold text-white orbitron">{{ number_format($stats['week_revenue'] ?? 0) }}</p>
                    <p class="text-gray-400 text-sm">VND</p>
                </div>
                <div class="w-12 h-12 bg-purple-500 rounded-lg flex items-center justify-center">
                    <span class="text-xl">📊</span>
        ...
        <div class="admin-card rounded-xl p-6 bg-gradient-to-br from-orange-500 bg-opacity-10 border-orange-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-300 text-sm">Doanh thu tháng này</p>
                    <p class="text-2xl font-bold text-white orbitron">{{ number_format($stats['month_revenue'] ?? 0) }}</p>
                    <p class="text-gray-400 text-sm">VND</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="admin-card rounded-xl p-6">
            ...
            <div class="bg-gray-800 bg-opacity-50 rounded-lg p-4">
                <div class="h-56 flex items-end justify-around gap-2">
                    @if(!empty($stats['revenue_chart_data']['values']))
                        @foreach($stats['revenue_chart_data']['values'] as $index => $value)
                            @php
                                $percentage = $stats['revenue_chart_data']['max'] > 0 ? ($value / $stats['revenue_chart_data']['max']) * 100 : 0;
                            ...
                            <div class="flex-1 flex flex-col items-center group">
                                ...
                                <div class="w-full h-48 flex items-end justify-center mb-2 relative">
                                    ...
                                    <div class="w-full rounded-t-lg transition-all duration-500 hover:brightness-125 cursor-pointer chart-bar relative"
                                        ...
                                        <div class="absolute -top-16 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs rounded-lg py-2 px-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10 pointer-events-none">
                                            <div class="font-bold text-center">{{ number_format($value) }} VND</div>
                                ...
                                <div class="text-white text-sm font-bold">
                                    {{ $stats['revenue_chart_data']['labels'][$index] ?? '' }}
                                </div>
                            </div>
                        @endforeach
                    @else
            ...
            <div class="mt-4 grid grid-cols-2 gap-3">
                <div class="bg-green-500 bg-opacity-20 rounded-lg p-3 border border-green-500 border-opacity-30">
                    ...
                    <div class="text-white font-bold text-sm">{{ number_format($stats['admin_revenue']) }} VND</div>
                </div>
                @if($stats['revenue_chart_data']['max'] > 0)
                    <div class="bg-blue-500 bg-opacity-20 rounded-lg p-3 border border-blue-500 border-opacity-30">
                        ...
                        <div class="text-white font-bold text-sm">{{ number_format($stats['revenue_chart_data']['max']) }} VND</div>
                    </div>
                @endif
            </div>
        </div>
        ...
        <!-- Top Sellers -->
        <div class="admin-card rounded-xl p-6">
            <h3 class="text-xl font-bold text-white mb-4 orbitron">🏆 Top Sellers (Doanh thu)</h3>
            @if(empty($stats['top_sellers']))
                <div class="text-center py-8">
                    <p class="text-gray-300">Chưa có seller nào có doanh thu</p>
                </div>
            @else
                <div class="space-y-4">
                    @php
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="text-white font-medium">{{ $seller['seller_name'] }}</p>
                                <p class="text-gray-300 text-sm">{{ $seller['transactions'] }} giao dịch</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="text-green-400 font-medium">{{ number_format($seller['revenue']) }} VND</span>
                            <p class="text-gray-400 text-xs">Hoa hồng</p>
                        </div>
                    </div>
                    @endforeach
                </div>
    ...
    <div class="mb-6">
        <div class="admin-card rounded-xl p-6">
            <h3 class="text-xl font-bold text-white mb-4 orbitron">💳 Giao dịch gần nhất</h3>
            @if(empty($stats['recent_transactions']))
                <div class="text-center py-8">
                    <p class="text-gray-300">Chưa có giao dịch nào</p>
                </div>
            @else
                <div class="overflow-x-auto">
                    <table class="min-w-full">
                        <thead class="bg-gray-800 bg-opacity-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-300">Sản phẩm</th>
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-300">Seller</th>
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-300">Giá</th>
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-300">Hoa hồng</th>
                                <th class="px-4 py-3 text-left text-sm font-medium text-gray-300">Thời gian</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($stats['recent_transactions'] as $transaction)
                            <tr class="border-b border-gray-700 last:border-none hover:bg-gray-800 bg-opacity-30">
                                <td class="px-4 py-3 text-sm text-white">{{ $transaction['product_name'] }}</td>
                                <td class="px-4 py-3 text-sm text-gray-300">{{ $transaction['seller_name'] }}</td>
                                <td class="px-4 py-3 text-sm text-blue-400 font-medium">{{ number_format($transaction['price']) }} VND</td>
                                <td class="px-4 py-3 text-sm text-green-400 font-medium">{{ number_format($transaction['admin_commission']) }} VND</td>
                                <td class="px-4 py-3 text-sm text-gray-400">{{ $transaction['date'] }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
