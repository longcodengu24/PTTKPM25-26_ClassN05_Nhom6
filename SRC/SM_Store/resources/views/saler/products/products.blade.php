            ...
            <div id="products" class="admin-content active px-6 pb-6">
                <div class="admin-card rounded-xl p-6">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('saler.products.create') }}" class="bg-blue-500 hover:bg-blue-600 px-6 py-3 rounded-lg text-white inter font-semibold">
                            + Thêm Sheet Mới
                        </a>
                    </div>
                                ...
                                <tr class="table-row">
                                    <td class="py-4">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div>
                                                <p class="text-white font-semibold inter">{{ $product['name'] ?? 'Chưa có tên' }}</p>
                                                <p class="text-blue-200 text-sm inter">{{ $product['author'] ?? 'Chưa rõ tác giả' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-4 text-white inter">{{ $product['transcribed_by'] ?? 'Seller' }}</td>
                                    <td class="py-4 text-white inter">{{ $product['country_region'] ?? 'Việt Nam' }}</td>
                                    <td class="py-4 text-white inter">{{ number_format($product['price'] ?? 0) }}đ</td>
                                    <td class="py-4 text-white inter">{{ $product['downloads_count'] ?? 0 }}</td>
                                    <td class="py-4">
                                    ...
                                    <td class="py-4">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('saler.products.edit', $product['id'] ?? '') }}" class="bg-yellow-500 hover:bg-yellow-600 px-3 py-1 rounded text-white text-sm">Sửa</a>
                                            <button onclick="confirmDelete('{{ $product['id'] ?? '' }}', '{{ addslashes($product['name'] ?? 'sản phẩm') }}')" class="bg-red-500 hover:bg-red-600 px-3 py-1 rounded text-white text-sm">Xóa</button>
                                        </div>
                                    </td>
                                </tr>
                                @empty
