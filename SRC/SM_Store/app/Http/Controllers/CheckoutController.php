...
class CheckoutController extends Controller
{
    ...
    public function processCheckout(Request $request)
    {
        try {
            ...
            // TEMPORARY DISABLED FOR TESTING - Cho phép mua lại sản phẩm đã mua để test
            /*
            ...
            */
            Log::info('⚠️ PURCHASE VALIDATION DISABLED - Allowing repurchase for testing');
            ...
            foreach ($cartItems as $item) {
                ...
                // ✅ LẤY THÔNG TIN SẢN PHẨM THẬT TỪ DATABASE
                ...
                // ✅ LẤY SELLER_ID TỪ SẢN PHẨM THẬT TRONG FIRESTORE
                $sellerId = $product['seller_id'] ?? null;
                
                if (!$sellerId) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Sản phẩm "' . $item['name'] . '" không có thông tin người bán (seller_id)'
                    ], 400);
                }
                ...
                // ✅ LẤY THÔNG TIN SELLER VÀ CỘNG XU
                if (!isset($sellersUpdated[$sellerId])) {
                    ...
                    if (!$seller) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Không tìm thấy thông tin người bán (seller_id: ' . $sellerId . ') cho sản phẩm "' . $item['name'] . '"'
                        ], 404);
                    }
